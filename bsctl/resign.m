...
int ResignSystemExecutables(bool resignAll)
{
    ...
    if([fm fileExistsAtPath:jbroot(@"/.zqbb")]) {
        NSArray* resignListAdd = [infoDict objectForKey:@"resign_list_zqbb_add"];
        if (resignListAdd) [ResignList addObjectsFromArray:resignListAdd];

        NSArray* resignListRm = [infoDict objectForKey:@"resign_list_zqbb_rm"];
        if (resignListRm) [ResignList removeObjectsInArray:resignListRm];
    }
